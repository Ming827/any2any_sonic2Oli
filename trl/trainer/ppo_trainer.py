...
class TRLPPOTrainer(PPOTrainer):  # noqa: F405
    ...
    def _init_trl(
        ...
        **kwargs,
        ...
        # TensorBoard writer (rank 0 only). Each call to self.log() flushes the
        # scalar dict to <log_dir>/tb/ so `tensorboard --logdir <log_dir>` works
        # without depending on wandb being logged in.
        self.tb_writer = None
        if log_dir is not None and (
            ...
            try:
                ...
                tb_dir = os.path.join(str(log_dir), "tb")
    ...
    def _init_config(self):
        ...
        # ---- entropy_coef decay schedule (wbt_lite_mlp-style) ----
        # Multiplicative decay applied every `entropy_decay_interval` iterations,
        # floored at `entropy_decay_min`. The std parameter is *not* directly
        # decayed; it stays a learnable nn.Parameter, but with shrinking entropy
        # bonus PPO naturally pulls the std down over iterations. Use this to
        # get the "init_noise_std=1.0 → slow monotonic decrease" pattern.
        self.entropy_decay = self.config.get("entropy_decay", False)
        self.entropy_decay_rate = float(self.config.get("entropy_decay_rate", 0.95))
        self.entropy_decay_interval = int(self.config.get("entropy_decay_interval", 500))
        self.entropy_decay_min = float(self.config.get("entropy_decay_min", 0.001))
    ...
    def _rollout_step(self, model, obs_dict):
        ...
        with torch.no_grad():
            ...
            # ---- DEBUG: linear-leak diagnostic ----
            if int(os.environ.get("MEM_DEBUG", "0")) and self.state.global_step % 50 == 0:
                try:
                ...
                print(
                    f"[mem] iter={self.state.global_step:6d}  "
                    f"alloc={torch.cuda.memory_allocated()/1e9:6.2f}GB  "
                    f"reserved={torch.cuda.memory_reserved()/1e9:6.2f}GB  "
                    f"rss={rss_gb:6.2f}GB  "
                    f"py_tensors={n_tensors:7d}  "
                    f"log_history={len(self.state.log_history):5d}  "
                    f"ep_infos={len(self.ep_infos):4d}",
                    flush=True,
                )
            # ---------------------------------------
    ...
    def train(self):
        ...
        for batch_idx in range(1, args.num_total_batches + 1):
            ...
            # Mirrors rsl_rl_lib/rsl_rl/runners/on_policy_runner.py:217-219.
            # Every `entropy_decay_interval` iterations (and not on the very
            # first), multiply entropy_coef by `entropy_decay_rate`, floored at
            # `entropy_decay_min`. Drives the natural decrease of action std
            # without forcibly clamping the std parameter.
            if (
                self.entropy_decay
                and batch_idx > 1
                and (batch_idx - 1) % self.entropy_decay_interval == 0
            ):
                new_coef = max(
                    self.entropy_decay_rate * self.entropy_coef,
                    self.entropy_decay_min,
                )
                if new_coef != self.entropy_coef:
                    self.entropy_coef = new_coef
                    accelerator.print(
                        f"[entropy_decay] iter {batch_idx}: entropy_coef -> {self.entropy_coef:.6f}"
                    )
    ...
    def log(self, logs: dict[str, float], start_time: float | None = None) -> None:
        ...
        # Convert tensors to plain Python scalars before appending so we don't
        # leak computation graph references through self.state.log_history,
        # which is unbounded by HF Trainer convention.
        scalar_logs = {}
        for k, v in logs.items():
            if isinstance(v, torch.Tensor):
                if v.numel() == 1:
                    scalar_logs[k] = v.detach().item()
                else:
                    scalar_logs[k] = v.detach().mean().item()
            else:
                scalar_logs[k] = v
        output = {**scalar_logs, **{"step": self.state.global_step}}  # noqa: PIE800
        self.state.log_history.append(output)
        # Cap history length to prevent unbounded CPU memory growth over long runs.
        max_log_history = 1000
        if len(self.state.log_history) > max_log_history:
            del self.state.log_history[: len(self.state.log_history) - max_log_history]

        # Mirror scalar metrics to TensorBoard (rank 0 only; writer is None elsewhere).
        if self.tb_writer is not None:
            ...
            for k, v in logs.items():
                ...
                if isinstance(v, (int, float)):
                    ...
                    self.tb_writer.add_scalar(k, v.item(), step)
