    ...
    def _post_eval_env_step(self, actor_state):
        ...
        if self.curr_steps >= curr_max or self.terminate_state.sum() == self.env.num_envs:
            ...
            if self.env_eval_loop_idx >= self.num_total_env_eval_loops:
                ...
                # The subset names above are written against the canonical G1
                # 14-body layout. Other robots (e.g. Oli) share this ordering
                # but rename some bodies (pelvis->base_link,
                # torso_link->waist_pitch_link, *_wrist_yaw_link->*_wrist_roll_link),
                # so resolve each subset by canonical position when a name is
                # not present verbatim. G1 keeps the exact original behaviour.
                ...
                def _subset_indices(subset_names):
                    if all(name in body_names for name in subset_names):
                        return [body_names.index(name) for name in subset_names]
                    if len(body_names) == len(canonical_body_names):
                        # Same layout/order, different names -> map by position.
                        return [canonical_body_names.index(name) for name in subset_names]
                    raise ValueError(
                        f"Eval subset {subset_names} not found in body_names {body_names}"
                    )

                # Get indices for subsets
                legs_indices = _subset_indices(legs_subset_names)
                vr_3points_indices = _subset_indices(vr_3points_subset_names)
                other_upper_bodies_indices = _subset_indices(other_upper_bodies_subset_names)
                foot_indices = _subset_indices(foot_subset_names)
                # Extract subset data
                ...
                # Fall back to the vendored pure-numpy copy when smpl_sim is not
                # installed in this environment (produces identical metrics).
                try:
                    from smpl_sim.smpllib.smpl_eval import compute_metrics_lite
                except ModuleNotFoundError:
                    from gear_sonic.trl.callbacks.smpl_eval_lite import compute_metrics_lite
