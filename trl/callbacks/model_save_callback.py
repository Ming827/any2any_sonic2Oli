...
class ModelSaveCallback(TrainerCallback):
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        """Save an initial checkpoint at t=0 before any optimization step.

        Useful as a baseline for diff/delta analysis against the post-training
        state, and ensures ``last.pt`` is valid even if training crashes before
        reaching ``save_last_frequency``.
        """
        ...
        if model is None or env is None:
            logger.warning("on_train_begin: model/env unavailable, skipping initial checkpoint")
            return
        ...
        ModelSaveCallback.save_checkpoint(
            model, optimizer, lr_scheduler, state, env_state_dict, args,
            f"{self.save_dir}/model_0.pt",
        )
        if self.save_last_frequency and self.save_last_frequency > 0:
            ModelSaveCallback.save_checkpoint(
                model, optimizer, lr_scheduler, state, env_state_dict, args,
                f"{self.save_dir}/last.pt",
            )
        logger.info(f"on_train_begin: saved initial checkpoint to {self.save_dir}/model_0.pt")
        ...
        if state.is_world_process_zero and not env.is_evaluating:
            ...
            if not self.save_last_only and state.global_step % self.save_frequency == 0:
                ...
                ModelSaveCallback.save_checkpoint(
                    ...
                    env_state_dict,
                    args,
                    f"{self.save_dir}/model_{state.global_step}.pt",
                )

            # Always save last checkpoint every save_last_frequency steps
            # (set save_last_frequency<=0 in yaml to disable last.pt entirely)
            if (
                self.save_last_frequency
                and self.save_last_frequency > 0
                and state.global_step % self.save_last_frequency == 0
            ):
                env_state_dict = env.get_env_state_dict()
                ModelSaveCallback.save_checkpoint(
                    model,
    ...
    def export_policy_to_onnx(self, env, state, model):
        ...
        onnx_step_path = os.path.join(
            env.config.experiment_dir, "exported", f"model_{state.global_step}.onnx"
        )
