...
class BaseModule(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # Activation-memory checkpointing for the underlying nn.Sequential.
        # When True (and self.training and module has trainable params), forward
        # uses torch.utils.checkpoint.checkpoint_sequential — splitting the
        # network into ``segments`` chunks and re-running each chunk's forward
        # during backward instead of caching its activations. Trades ~30% extra
        # forward FLOPs (during backward) for ~30-50% activation memory savings.
        # Cheap to combine with LoRA: backbone has no gradient anyway, so the
        # only "extra forward cost" is on the LoRA injection sites, which are
        # tiny relative to the main Linear layers.
        # No-op outside training (rollout / eval) so sample collection isn't
        # slowed down.
    ...
    def forward_without_hidden_state(self, input):
        ...
        """
        if self._should_grad_checkpoint(input):
            return self._grad_checkpointed_forward(input)
        return self.module(input)

    def _should_grad_checkpoint(self, input):
        """Decide whether to apply gradient checkpointing on this forward call.

        Skip when:
          - Flag is disabled (default).
          - Module is not in training mode (rollout/eval — no backward, so
            checkpointing would just waste forward FLOPs without saving anything).
          - Underlying module isn't an nn.Sequential (checkpoint_sequential
            assumes that structure).
          - No trainable params under self.parameters() — fully frozen path,
            backward won't fire, checkpoint provides no benefit.
        """
        if not self.use_gradient_checkpointing:
            return False
        if not self.training:
            return False
        if not isinstance(self.module, nn.Sequential):
            return False
        if not any(p.requires_grad for p in self.parameters()):
            return False
        return True

    def _grad_checkpointed_forward(self, input):
        """Run self.module via torch.utils.checkpoint.checkpoint_sequential.

        ``checkpoint_sequential`` requires the input tensor to have
        ``requires_grad=True`` for autograd to fire on the recomputed forward
        path. The raw obs tensor coming from the environment usually has
        ``requires_grad=False``; we make a grad-enabled view via
        ``detach().requires_grad_(True)`` (cheap — same storage, no copy).
        """
        ...
        return checkpoint_sequential(
            self.module,
            self.gradient_checkpointing_segments,
            input,
            use_reentrant=False,
        )
    ...
    def forward(self, input, **kwargs):
        ...
        if self._should_grad_checkpoint(input):
            output = self._grad_checkpointed_forward(input)
        else:
