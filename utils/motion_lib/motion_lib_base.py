...
# Repo root = parent of `utils/`. We resolve relative paths against this so
# `motion_file=data/oli_motion/...` works regardless of which cwd the entry
# script was launched from (e.g. `python sonic_wbt/train_agent_trl.py ...`
# from one level up).
# This file lives at <repo>/utils/motion_lib/motion_lib_base.py — three
# dirname() calls walk up to <repo>.
_REPO_ROOT = osp.dirname(osp.dirname(osp.dirname(osp.abspath(__file__))))


def _resolve_motion_path(p):
    """If ``p`` is relative and not found at cwd, retry against the repo root.

    Tries, in order:
      1. ``p`` as-is (absolute, or relative to cwd)
      2. ``<repo_root>/<p>`` — relative resolved against repo root
      3. ``<repo_root>/<p with leading "gear_sonic/" stripped>`` — handles
         legacy configs where ``ASSET_DIR = "gear_sonic/data/assets"`` was
         baked into the saved config; running from inside ``gear_sonic/``
         (whose name on disk is ``gear_sonic``) makes the literal prefix
         redundant.

    Returns the original input on full miss so the downstream "file not
    found" error surfaces with a meaningful message.
    """
    if p is None or not isinstance(p, str) or p in ("dummy", "zeros"):
        return p
    if osp.isabs(p) or osp.exists(p):
        return p
    candidate = osp.join(_REPO_ROOT, p)
    if osp.exists(candidate):
        return candidate
    # Legacy: strip leading "gear_sonic/" or "<basename(repo_root)>/"
    repo_basename = osp.basename(_REPO_ROOT)
    for prefix in ("gear_sonic/", f"{repo_basename}/"):
        if p.startswith(prefix):
            stripped = p[len(prefix):]
            candidate = osp.join(_REPO_ROOT, stripped)
            if osp.exists(candidate):
                return candidate
    return p
...
class MotionLibBase:
    def __init__(self, motion_lib_cfg, num_envs, device):
        ...
        # Resolve the MJCF skeleton path the same way as motion files: legacy
        # configs saved at training time have ``gear_sonic/data/assets/...``
        # baked in; running from inside gear_sonic/ that becomes a doubled
        # path. ``_resolve_motion_path`` strips the redundant prefix and
        # falls back to the repo root.
        raw_skeleton_path = str(
            Path(self.m_cfg.asset.assetRoot) / self.m_cfg.asset.assetFileName
        )
        skeleton_file = _resolve_motion_path(raw_skeleton_path)
        if skeleton_file != raw_skeleton_path:
            logger.info(
                f"skeleton path resolved against repo root: "
                f"{raw_skeleton_path} -> {skeleton_file}"
            )
        self.skeleton_tree = skeleton.SkeletonTree.from_mjcf(skeleton_file)
        logger.info(f"Loaded skeleton from {skeleton_file}")
        # Resolve motion_file (and the other *_motion_file fields below)
        # against the repo root if a relative path was provided — so the
        # CLI override `motion_file=data/oli_motion/...` works regardless of
        # the cwd from which the entry script was launched.
        resolved_motion_file = _resolve_motion_path(self.m_cfg.motion_file)
        if resolved_motion_file != self.m_cfg.motion_file:
            logger.info(
                f"motion_file resolved against repo root: "
                f"{self.m_cfg.motion_file} -> {resolved_motion_file}"
            )
        self.m_cfg.motion_file = resolved_motion_file
        ...
        self.smpl_data = None
        smpl_motion_file = _resolve_motion_path(motion_lib_cfg.get("smpl_motion_file", None))
        ...
        self.soma_data = None
        soma_motion_file = _resolve_motion_path(motion_lib_cfg.get("soma_motion_file", None))
        ...
        object_motion_file = _resolve_motion_path(motion_lib_cfg.get("object_motion_file", None))
    ...
    def _apply_dirtylist(self):
        """Drop motions enumerated in the optional dirtylist text file.

        File format (read from ``self.m_cfg.dirtylist_path``):
          - One entry per line.
          - ``#`` and blank lines are skipped.
          - Each entry can be a bare key, a relative path under the motion
            root, or an absolute path. The matcher always computes
            ``basename without extension`` for both the dirtylist entry and
            each loaded motion's key (which motion_lib already builds the
            same way), so all three forms work uniformly.

        Examples (any of these match the same motion ``walk_clip_03``):
            walk_clip_03
            walk_clip_03.npy
            AMASS/npy_filtered_0/walk_clip_03.npy
            /abs/path/oli_motion/AMASS/npy_filtered_0/walk_clip_03.npy

        Missing dirtylist file → warn and skip; nothing else.
        """
        dirtylist_path = _resolve_motion_path(self.m_cfg.get("dirtylist_path", None))
        if not dirtylist_path:
            return
        if not osp.isfile(dirtylist_path):
            logger.warning(
                f"dirtylist_path does not exist: {dirtylist_path}; ignoring."
            )
            return

        bad_keys: set[str] = set()
        # Also keep the original full strings so we can additionally match
        # against each motion's stored absolute path — useful when the same
        # basename appears in multiple subdirs and only one is bad.
        bad_paths: set[str] = set()
        with open(dirtylist_path) as f:
            for raw in f:
                line = raw.strip()
                if not line or line.startswith("#"):
                    continue
                # Always derive the key for matching against motion_lib's
                # ``osp.splitext(osp.basename(f))[0]`` keys.
                bad_keys.add(osp.splitext(osp.basename(line))[0])
                # Whole line is a path-like form? Keep it for direct path match.
                if "/" in line or line.endswith((".npy", ".pkl")):
                    bad_paths.add(line)

        if not bad_keys and not bad_paths:
            return

        n_before = len(self._motion_data_load)
        keys_to_remove = []
        # The path-suffix fallback (endswith over every bad_paths entry per
        # motion) is O(N*M) and turns into hours when both N and M are large
        # (e.g. 500k motions × 8k dirty paths). For most real-world dirtylists
        # the basename → key match via ``bad_keys`` already catches every
        # entry. Only fall back to path checks when the dirtylist is small
        # enough that the slower scan stays cheap.
        do_path_fallback = len(bad_paths) <= 200
        for k, v in self._motion_data_load.items():
            if k in bad_keys:
                keys_to_remove.append(k)
                continue
            if not do_path_fallback or not isinstance(v, dict):
                continue
            full_path = v.get("path")
            if full_path is None:
                continue
            if full_path in bad_paths:
                keys_to_remove.append(k)
                continue
            for bp in bad_paths:
                if not bp.startswith("/"):
                    if full_path.endswith("/" + bp) or full_path == bp:
                        keys_to_remove.append(k)
                        break

        for k in keys_to_remove:
            del self._motion_data_load[k]

        logger.info(
            f"dirtylist '{dirtylist_path}': removed {len(keys_to_remove)} motions "
            f"({len(bad_keys)} key/basename entries, {len(bad_paths)} path-like entries; "
            f"{len(self._motion_data_load)} of {n_before} remain)"
        )

    @staticmethod
    def _resolve_scan_roots(motion_file: str, include_subdirs):
        """Translate ``include_subdirs`` config into concrete scan roots.

        - ``None`` / empty → return ``[motion_file]`` (full recursive scan,
          same as before this option existed).
        - list of strings → join each onto ``motion_file``; warn and skip
          entries that don't exist on disk; raise if **all** entries miss
          (clearly a misconfiguration, better to fail loud than silently
          load nothing).
        """
        ...
        for sub in missing:
            logger.warning(
                f"include_subdirs entry '{sub}' not found under {motion_file}; skipping."
            )
        if not roots:
            raise FileNotFoundError(
                f"include_subdirs={include_subdirs} but none of these exist under "
                f"{motion_file}. Either fix the names or remove include_subdirs."
            )
        logger.info(
            f"include_subdirs active: scanning {len(roots)}/{len(include_subdirs)} "
            f"subdirs under {motion_file}: {[osp.basename(r) for r in roots]}"
        )
        return roots
    ...
    def load_data(self, motion_file):
        if osp.isfile(motion_file):
            ...
            if self.debug:
                ...
                # Default: scan only .npy files (wbt_lite_mlp / Oli native
                # format, converted on-the-fly to body_states by
                # convert_npy_to_dict_*).
                # Set ``motion_lib_cfg.allow_pkl_in_dir_scan: true`` to also
                # pick up .pkl files (G1 SONIC bones_seed style — these go
                # through fk_batch). Only enable for G1 / native SONIC paths
                # where pkl is the canonical motion format.
                #
                # Subset by subdirectory: set ``motion_lib_cfg.include_subdirs``
                # to a list of subfolder names (relative to ``motion_file``)
                # to restrict scanning to those folders only — useful when
                # the dataset root has dozens of categories but you only want
                # to train on a few. Missing subdirs are warned and skipped;
                # leave unset / None to keep the full recursive scan.
                include_subdirs = self.m_cfg.get("include_subdirs", None)
                scan_roots = self._resolve_scan_roots(motion_file, include_subdirs)

                def _glob_ext(ext: str):
                    out = {}
                    for root in scan_roots:
                        for f in glob.glob(osp.join(root, "**", f"*.{ext}"), recursive=True):
                            out[osp.splitext(osp.basename(f))[0]] = {
                                "path": f,
                                **({"_is_npy": True} if ext == "npy" else {}),
                            }
                    return out

                npy_files = _glob_ext("npy")
                # Auto-fallback to pkl when the directory has no npy at all.
                # Saved ckpt config.yaml from a G1 SONIC run may not carry
                # the explicit allow_pkl_in_dir_scan=true flag; rather than
                # crash with IndexError downstream, just scan pkl too in
                # that case. Mixed dirs (npy + pkl) still need the explicit
                # flag — that's where the safety mattered originally.
                allow_pkl = bool(self.m_cfg.get("allow_pkl_in_dir_scan", False))
                if not allow_pkl and not npy_files:
                    allow_pkl = True
                if allow_pkl:
                    pkl_files = {}
                    for root in scan_roots:
                        for f in glob.glob(osp.join(root, "**", "*.pkl"), recursive=True):
                            if f.endswith("metadata.pkl"):
                                continue
                            pkl_files[osp.splitext(osp.basename(f))[0]] = {"path": f}
                    # PKL takes precedence on key collision (it's pre-baked).
                    self._motion_data_load = {**npy_files, **pkl_files}
                    if npy_files and pkl_files:
                        logger.info(
                            f"Found {len(npy_files)} .npy + {len(pkl_files)} .pkl in "
                            f"{motion_file} ({len(self._motion_data_load)} unique keys)"
                        )
                    elif pkl_files:
                        logger.info(
                            f"Found {len(pkl_files)} .pkl files in {motion_file}"
                        )
                    else:
                        logger.info(
                            f"Found {len(npy_files)} .npy files in {motion_file}"
                        )
                else:
                    self._motion_data_load = npy_files
                    logger.info(
                        f"Found {len(npy_files)} .npy files in {motion_file} "
                        "(.pkl scan disabled — set motion_lib_cfg.allow_pkl_in_dir_scan=true to enable)"
                    )

            # Optional dirtylist: drop bad/broken motions before any further
            # processing. ``m_cfg.dirtylist_path`` points at a plain text
            # file; one entry per line, ``#`` for comments. An entry is
            # treated as an absolute path if it starts with ``/``, otherwise
            # as a motion key (basename without extension).
            self._apply_dirtylist()
        ...
        # Keep only a random *fraction* of the dataset. Unlike
        # ``max_unique_motions`` (a fixed count), ``subsample_ratio`` selects a
        # proportion in (0, 1) of the currently loaded motions. A dedicated RNG
        # seeded by ``subsample_seed`` (default 0) keeps the chosen subset
        # reproducible across runs and identical across ranks in multi-GPU jobs.
        subsample_ratio = self.m_cfg.get("subsample_ratio", None)
        if subsample_ratio is not None and 0.0 < subsample_ratio < 1.0:
            import random

            keys = sorted(data_list.keys())  # Sort for determinism, then sample
            num_to_keep = max(1, int(round(len(keys) * subsample_ratio)))
            rng = random.Random(self.m_cfg.get("subsample_seed", 0))
            selected = rng.sample(keys, num_to_keep)
            data_list = {k: data_list[k] for k in selected}
            print(  # noqa: T201
                f"Subsampled to {num_to_keep} random motions "
                f"({subsample_ratio:.1%} of {len(keys)})"
            )
    ...
    def load_motion_with_skeleton(
        ...
        for f in pbar:
            ...
            if "path" in curr_file:
                if curr_file.get("_is_npy", False):
                    # On-the-fly conversion of wbt_lite_mlp .npy to SONIC format.
                    # Dispatch by motion-lib DOF count: 31 → Oli-native, else G1.
                    num_dof_mapping = len(self.m_cfg.mujoco_to_isaaclab_dof)
                    if num_dof_mapping == 31:
                        from gear_sonic.utils.npy_to_sonic_pkl import convert_npy_to_dict_oli
                        curr_file = convert_npy_to_dict_oli(
                            curr_file["path"],
                            target_fps=self.target_fps,
                        )
                    else:
                        from gear_sonic.utils.npy_to_sonic_pkl import convert_npy_to_dict
                        curr_file = convert_npy_to_dict(
                            curr_file["path"],
                            target_fps=self.target_fps,
                        )
                else:
            ...
            # Short-circuit FK when the motion dict already carries precomputed
            # body_states (from IsaacLab sim). Skipping FK avoids approximating
            # body poses with a surrogate MJCF skeleton whose link geometry may
            # not match the real robot. See npy_to_sonic_pkl.convert_npy_to_dict_oli.
            skip_fk = (
                "body_pos_w" in curr_file
                and "body_quat_w" in curr_file
                and "body_lin_vel_w" in curr_file
                and "body_ang_vel_w" in curr_file
                and "dof_vel" in curr_file
            )

            if self.mesh_parsers is not None and not skip_fk:
                ...
                # Build curr_motion directly from the precomputed body fields.
                # Shapes use a leading (1, T, ...) batch dim to match fk_batch
                # output; the finalize block (squeeze) downstream strips it.
                body_pos_w = to_torch(curr_file["body_pos_w"])[start:end][None, ...].float()
                body_quat_w = to_torch(curr_file["body_quat_w"])[start:end][None, ...].float()
                body_lin_vel_w = to_torch(curr_file["body_lin_vel_w"])[start:end][None, ...].float()
                body_ang_vel_w = to_torch(curr_file["body_ang_vel_w"])[start:end][None, ...].float()
                dof_pos = to_torch(curr_file["dof"])[start:end][None, ...].float()
                dof_vels = to_torch(curr_file["dof_vel"])[start:end][None, ...].float()

                _, T_eff, NB, _ = body_pos_w.shape
                curr_motion = easydict.EasyDict(
                    {
                        "global_translation": body_pos_w,
                        "global_rotation": body_quat_w,
                        "global_velocity": body_lin_vel_w,
                        "global_angular_velocity": body_ang_vel_w,
                        "global_root_velocity": body_lin_vel_w[:, :, 0, :],
                        "global_root_angular_velocity": body_ang_vel_w[:, :, 0, :],
                        # local_rotation is unused when dof_pos is present, but
                        # the aggregation step (load_motions) still references
                        # it — fill with identity quaternions (xyzw = (0,0,0,1)).
                        "local_rotation": torch.zeros(1, T_eff, NB, 4).float(),
                        "dof_pos": dof_pos,
                        "dof_vels": dof_vels,
                        # Mirror fk_batch's return_dict.fps so downstream code
                        # (e.g. motion_lib_base:1339 motion_fps = int(curr_motion.fps * scale))
                        # doesn't AttributeError on the EasyDict.
                        "fps": int(self.target_fps),
                    }
                )
                curr_motion["local_rotation"][..., 3] = 1.0
            else:
                curr_motion = None
    ...
    def init_adaptive_sampling(self):
        ...
        # Indices of motions that fail to probe (corrupt npy / pkl, missing
        # required keys, IO errors). Collected here and removed from the
        # motion pool after the loop, before adaptive-sampling structures
        # downstream of this function are built.
        bad_motion_indices: list[int] = []
        ...
        for i, motion_key in enumerate(self._motion_data_keys):
            ...
            # For wbt_lite_mlp Oli npy (_is_npy=True) we mirror convert_npy_to_dict_oli's
            # contract: trust target_fps when the file has no usable fps field, and never
            # resample on load. Without this, init_adaptive_sampling and the loader can
            # disagree on per-motion frame counts (assertion at 2883 catches it).
            if "fps" not in motion_data.keys():  # noqa: SIM118
                motion_data["fps"] = (
                    float(self.target_fps) if motion_data.get("_is_npy", False) else 30.0
                )
            ...
            try:
                if "length" in motion_data:
                    ...
                    # Directory mode: lazy-load the file to get frame count and fps
                    if motion_data.get("_is_npy", False):
                        # wbt_lite_mlp npy: read shape; only override fps if the file
                        # explicitly carries a non-None value, else stick with target_fps.
                        npy_data = np.load(motion_data["path"], allow_pickle=True).item()
                        # Per the wbt_lite_mlp contract dof_pos_vel and body_states
                        # share the same first dim T. Use whichever is present;
                        # if neither, mark this motion as bad and skip — it will
                        # be removed from the pool after the loop.
                        if "dof_pos_vel" in npy_data:
                            original_num_frames = npy_data["dof_pos_vel"].shape[0]
                        elif "body_states" in npy_data:
                            original_num_frames = npy_data["body_states"].shape[0]
                        else:
                            logger.warning(
                                f"[motion_lib] Skipping bad npy (no 'dof_pos_vel' "
                                f"or 'body_states'): {motion_data['path']} — "
                                f"keys: {list(npy_data.keys())}"
                            )
                            bad_motion_indices.append(i)
                            continue
                        # NOTE: deliberately ignore npy_data["fps"]. The
                        # converter (convert_npy_to_dict_oli) also ignores
                        # it and labels output as target_fps (default 50).
                        # Reading real npy fps here while the converter
                        # doesn't would make init_adaptive_sampling and
                        # load_motions disagree on per-motion frame counts
                        # (assertion at ~line 3097 catches it).
                    else:
                ...
                # Any read / parse failure → log + skip; the motion will be
                # removed from the pool below. We deliberately do NOT raise:
                # the user's intent is to keep training going on the good
                # data and emit the bad paths so they can fix or dirtylist.
                logger.warning(
                    f"[motion_lib] Skipping bad motion '{motion_key}' "
                    f"(path={motion_data.get('path', '<no path>')}): {e}"
                )
                bad_motion_indices.append(i)
                continue
        ...
        # Drop bad motions from the pool so downstream sampling never picks
        # them. Updates the four parallel structures plus adp_samp_num_frames
        # to keep indices aligned.
        if bad_motion_indices:
            bad_keys = [str(self._motion_data_keys[i]) for i in bad_motion_indices]
            preview = bad_keys[:5]
            logger.warning(
                f"[motion_lib] Removing {len(bad_keys)} bad motion(s) from pool: "
                f"{preview}{'...' if len(bad_keys) > 5 else ''}"
            )
            bad_set = set(bad_motion_indices)
            keep_indices = [
                j for j in range(len(self._motion_data_keys)) if j not in bad_set
            ]
            self._motion_data_keys = self._motion_data_keys[keep_indices]
            self._motion_data_list = self._motion_data_list[keep_indices]
            for k in bad_keys:
                self._motion_data_load.pop(k, None)
            self._num_unique_motions = len(self._motion_data_keys)
            keep_idx_tensor = torch.tensor(
                keep_indices, dtype=torch.long, device=self._device
            )
            self.adp_samp_num_frames = self.adp_samp_num_frames[keep_idx_tensor].contiguous()
