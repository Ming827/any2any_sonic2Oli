...
class Humanoid_Batch:
    def __init__(self, cfg, device=torch.device("cpu")):
        ...
        # Resolve via the same fallback motion_lib_base uses, so legacy
        # ckpt configs with ``gear_sonic/data/assets/...`` literal paths
        # work regardless of cwd.
        from gear_sonic.utils.motion_lib.motion_lib_base import _resolve_motion_path

        raw_path = str(self.asset_root / self.asset_file)
        self.mjcf_file = _resolve_motion_path(raw_path)
        ...
        actuator_elem = tree.getroot().find("actuator")
        if actuator_elem is not None:
            motors = sorted([m.attrib["name"] for m in actuator_elem.findall("motor")])
        else:
            # Visualization-only MJCFs (e.g., HU_D04_01_vis.xml) omit <actuator>.
            # Fall back to the kinematic joint list, dropping any free joint
            # (root) so num_dof matches the actuated DOF count.
            motors = sorted(
                j.attrib["name"]
        ...
        assert len(motors) > 0, "No motors or actuated joints found in the mjcf file"
