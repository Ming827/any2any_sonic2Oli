...
class ManagerEnvWrapper:
    def __init__(self, env: "ManagerBasedEnv", config):
        ...
        # Initial viewer behavior: snap camera to the first robot ONCE on the
        # first reset (after the robot has actually been spawned and stepped),
        # then stay in free / world mode so the user can mouse-drag freely.
        # We defer to the first ``reset()`` because ``root_pos_w`` may not be
        # populated at wrapper-construction time. The 'F' key re-snaps later.
        self.viewer_focused = False
        self._initial_focus_done = False

        # Optional: dump action/obs/motion DOF & body orders to stdout.
        # Triggered by env var ``DUMP_ORDERS=1``. One-shot at wrapper init.
        # Useful for verifying that motion / proprio / action all line up
        # with the same articulation index convention.
        if os.environ.get("DUMP_ORDERS", "0") == "1":
            try:
                self._dump_orders()
            except Exception as e:  # noqa: BLE001
                logger.warning(f"[dump_orders] failed: {e}")

    def _dump_orders(self):
        """Print the joint / body orders used by action output, proprio
        observations, and motion-derived observations. Called once at init
        when ``DUMP_ORDERS=1``.
        """
        import sys
        ...
        _say("\n" + "=" * 78)
        _say("[dump_orders] Robot articulation")
        _say("=" * 78)
        _say(f"  num_joints={len(joint_names)}, num_bodies={len(body_names)}")

        # ---- 1. Action output joint order ----
        _say("\n--- 1. Action output joint order (DOF index → name) ---")
        for i, n in enumerate(joint_names):
            _say(f"  [{i:2d}] {n}")

        def _print_group(group_name: str, label: str):
            _say(f"\n--- {label} ---")
            try:
                obs_man = self.env.observation_manager
                term_names = obs_man._group_obs_term_names[group_name]  # noqa: SLF001
                # Per-term dims are kept as a list of shape-tuples in
                # ``_group_obs_term_dim``; ``_group_obs_dim`` is the
                # collapsed-per-group total only.
                term_dims_list = obs_man._group_obs_term_dim[group_name]  # noqa: SLF001
                cursor = 0
                import functools as _ft
                for tname, tshape in zip(term_names, term_dims_list):
                    if isinstance(tshape, int):
                        size = int(tshape)
                    else:
                        size = int(_ft.reduce(lambda a, b: a * b, tshape, 1))
                    shape_str = (
                        f"shape={tuple(tshape)}" if not isinstance(tshape, int) else ""
                    )
                    _say(
                        f"  {tname:<32} dim={size:5d}  → flat[{cursor}:{cursor + size}]  "
                        f"{shape_str}"
                    )
                    cursor += size
                _say(f"  total {group_name}_obs dim = {cursor}")
            except Exception as e:  # noqa: BLE001
                _say(f"  (failed to read {group_name} group: {e})")

        # ---- 2. Proprioception (actor_obs) layout ----
        _print_group("policy", "2. Proprioception (actor_obs) layout")
        _say(
            "  Each per-DOF segment (joint_pos / joint_vel / actions) is "
            "flat history × num_joints; the num_joints axis follows section 1."
        )

        # ---- 3. Critic obs layout ----
        _print_group("critic", "3. Critic obs (privileged) layout")

        # ---- 4. Motion-side joint / body orders ----
        _say("\n--- 4. Motion-side orders (motion_lib internal) ---")
        try:
            mc = self.motion_command
            ml = self._motion_lib
            _say(f"  motion_lib_num_dof    = {mc.motion_lib_num_dof}")
            _say(f"  robot_num_dof         = {mc.robot_num_dof}")
            _say(f"  has_dof_mismatch      = {mc.has_dof_mismatch}")
            _say(f"  anchor_body (motion)  = {mc.cfg.anchor_body}  "
                 f"→ motion slot {mc.motion_anchor_body_index}, "
                 f"robot body slot {mc.robot_anchor_body_index}")

            _say("\n  body_names (14-body tracked subset):")
            for i, n in enumerate(mc.cfg.body_names):
                robot_idx = mc.robot.body_names.index(n) if n in mc.robot.body_names else "?"
                _say(f"    [{i:2d}] {n:<28} robot.body_names[{robot_idx}]")

            _say(
                "\n  motion_lib.dof_pos shape = "
                f"{tuple(ml.dof_pos.shape) if hasattr(ml, 'dof_pos') else '(not loaded yet)'}, "
                "DOF axis follows robot.joint_names order (already permuted from "
                "MJCF→IsaacLab in motion_lib_base.py:1701)"
            )
            _say(
                "  motion_lib.body_pos_w shape = "
                f"{tuple(ml.body_pos_w.shape) if hasattr(ml, 'body_pos_w') else '(not loaded yet)'}, "
                "body axis follows the body_names list above"
            )

            # reward_point_body / vr_3point_body indices
            _say("\n  reward_point_body (motion → robot index pairs):")
            for i, n in enumerate(mc.cfg.reward_point_body):
                m_idx = mc.reward_point_body_indices_motion[i]
                r_idx = mc.reward_point_body_indices[i]
                _say(f"    [{i}] {n:<28} motion[{m_idx}] / robot[{r_idx}]")

            _say("\n  vr_3point_body:")
            for i, n in enumerate(mc.cfg.vr_3point_body):
                m_idx = mc.vr_3point_body_indices_motion[i]
                r_idx = mc.vr_3point_body_indices[i]
                _say(f"    [{i}] {n:<28} motion[{m_idx}] / robot[{r_idx}]")
        except Exception as e:  # noqa: BLE001
            _say(f"  (failed to read motion command: {e})")
    ...
    def reset(self, flatten_dict_obs=True):
        ...
        # One-shot: snap camera to robot[0] on the first reset, after which
        # the user can drag the viewport freely (no tracking).
        if not self._initial_focus_done and not self.config.headless:
            try:
                self.focusing_viewer()
            except Exception as e:  # noqa: BLE001
                logger.info(f"Initial camera snap to robot[0] skipped: {e}")
    ...
    def step(self, actions):
        ...
        if self._motion_lib is not None and self._motion_lib.use_adaptive_sampling:
            ...
            ] = self._motion_lib.adp_samp_num_episodes.min().detach()
            extras["to_log"][
                "adp_samp/num_episodes_max"
            ] = self._motion_lib.adp_samp_num_episodes.max().detach()
            extras["to_log"][
                "adp_samp/num_episodes_mean"
            ] = self._motion_lib.adp_samp_num_episodes.mean().detach()
            extras["to_log"][
                "adp_samp/num_failures_min"
            ] = self._motion_lib.adp_samp_num_failures.min().detach()
            extras["to_log"][
                "adp_samp/num_failures_max"
            ] = self._motion_lib.adp_samp_num_failures.max().detach()
            extras["to_log"][
                "adp_samp/num_failures_mean"
            ] = self._motion_lib.adp_samp_num_failures.mean().detach()
            extras["to_log"][
                "adp_samp/failure_rate_min"
            ] = self._motion_lib.adp_samp_failure_rate_raw.min().detach()
            extras["to_log"][
                "adp_samp/failure_rate_max"
            ] = self._motion_lib.adp_samp_failure_rate_raw.max().detach()
            extras["to_log"][
                "adp_samp/failure_rate_mean"
            ] = self._motion_lib.adp_samp_failure_rate_raw.mean().detach()
            ...
            if hasattr(self._motion_lib, "adp_sampling_active_prob"):
                ...
                extras["to_log"]["adp_samp/prob_max"] = prob.max().detach()
                extras["to_log"]["adp_samp/prob_min"] = prob.min().detach()
                extras["to_log"]["adp_samp/prob_mean"] = prob.mean().detach()
                extras["to_log"]["adp_samp/prob_max_over_uniform"] = (
                    prob.max() / uniform_prob
                ).detach()
                extras["to_log"]["adp_samp/effective_num_bins"] = (
                    1.0 / (prob**2).sum()
                ).detach()
                ...
                extras["to_log"]["adp_samp/num_concentrated_bins"] = (
                    (prob > 10 * uniform_prob).sum().float().detach()
                )
    ...
    def focusing_viewer(self):
        # Lock camera to TRACK the next robot (asset_root mode) — the robot
        # stays centered as it moves. Each F press advances to the next env
        # (env 0 → 1 → 2 → ... → 0). IsaacLab's ``_update_tracking_callback``
        # re-sets the camera every render frame using:
        #     cam_eye    = root_pos_w[env_index] + default_cam_eye
        #     cam_target = root_pos_w[env_index] + default_cam_lookat
        # So we just need to:
        #   1. cfg.origin_type = "asset_root", asset_name = "robot"
        #   2. cfg.env_index = focused env
        #   3. default_cam_eye   = [2, 2, 1.5]   (offset relative to robot)
        #      default_cam_lookat = [0, 0, 0]    (look AT robot center)
        # and the callback will keep the robot centered automatically.
        ctrl = self.env.viewport_camera_controller

        # Advance focus index. First-ever call lands on env 0; subsequent
        # F presses cycle through 1, 2, ..., num_envs-1, 0, ...
        if not getattr(self, "_focus_env_idx_initialized", False):
            self._focus_env_idx = 0
            self._focus_env_idx_initialized = True
        else:
            self._focus_env_idx = (self._focus_env_idx + 1) % self.num_envs

        env_idx = self._focus_env_idx

        # Camera offset relative to the tracked robot (eye position) and the
        # look-at offset (0 = robot's root). These become the controller's
        # ``default_cam_eye`` / ``default_cam_lookat`` and are added to the
        # robot's world position every render frame.
        ctrl.default_cam_eye = np.array([2.0, 2.0, 1.5])
        ctrl.default_cam_lookat = np.array([0.0, 0.0, 0.0])

        # Switch to asset_root tracking targeting "robot" at the chosen env.
        ctrl.cfg.env_index = env_idx
        ctrl.update_view_to_asset_root("robot")

        self.viewer_focused = True
        logger.info(
            f"Camera now TRACKING robot[{env_idx}] / {self.num_envs} (centered)"
        )
