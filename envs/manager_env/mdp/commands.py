...
class TrackingCommand(CommandTerm):
    ...
    def __init__(
        ...
        # Optional body-name aliases for cross-robot motion retargeting
        # (e.g., Oli has "base_link" instead of G1's "pelvis").
        # Maps motion body name → robot body name.
        body_aliases = getattr(env.cfg, "body_name_aliases", {}) or {}

        def to_robot(name: str) -> str:
            return body_aliases.get(name, name)

        self.robot_anchor_body_index = self.robot.body_names.index(to_robot(self.cfg.anchor_body))
        self.motion_anchor_body_index = self.cfg.body_names.index(self.cfg.anchor_body)
        self.vr_3point_body_indices = [
            self.robot.body_names.index(to_robot(name)) for name in self.cfg.vr_3point_body
        ]
        ...
        self.reward_point_body_indices = [
            self.robot.body_names.index(to_robot(name)) for name in self.cfg.reward_point_body
        ]
        ...
        robot_body_names_for_find = [to_robot(name) for name in self.cfg.body_names]
        self.body_indexes = torch.tensor(
            self.robot.find_bodies(robot_body_names_for_find, preserve_order=True)[0],
            dtype=torch.long,
            device=self.device,
        )
