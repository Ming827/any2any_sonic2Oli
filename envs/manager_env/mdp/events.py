...
class EventCfg:
    ...
    add_base_mass = None
    add_hand_mass = None
    random_inertias = None

    # reset
    random_joint_armature = None
    actuator_gains = None
...
def randomize_rigid_body_inertia(
    ...
    """Multiplicatively scale the inertia tensor of selected rigid bodies (startup-mode DR).

    Ported from wbt_lite_mlp.tasks.locomotion.mdp.events.randomize_rigid_body_inertia
    so gear_sonic can match wbt's domain-randomization profile (waist_pitch_link
    inertia ±10%).
    """
    ...
    inertias[env_ids[:, None], body_ids] *= math_utils.sample_uniform(
        scale[0], scale[1], size, device="cpu"
    )
