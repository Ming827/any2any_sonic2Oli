...
# Absolute path to the gear_sonic package's data/assets dir. Resolved
# from this file (parents[3] = gear_sonic root), so it works regardless
# of which cwd the entry script is launched from. The previous
# ``"gear_sonic/data/assets"`` relative form broke when running from
# inside gear_sonic itself (became ``gear_sonic/gear_sonic/...``).
ASSET_DIR = str(Path(__file__).resolve().parents[3] / "data" / "assets")
