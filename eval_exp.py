...
class CheckpointEvaluator:
            ...
            if eval_subdir.is_dir():
                ...
                if (
                    ...
                    try:
                        step_num = int(eval_subdir.name)
                        checkpoint_path = (
                            self.experiment_dir / f"model_{step_num}.pt"
                        )
    ...
    def find_checkpoints(self) -> list[Path]:
        ...
        New naming: ``model_<step>.pt`` (e.g. ``model_2500.pt``).
        Legacy naming: ``model_step_<step:06d>.pt`` is also globbed for
        backward compatibility with old experiment dirs.
        """
        new_pattern = str(self.experiment_dir / "model_*.pt")
        legacy_pattern = str(self.experiment_dir / "model_step_*.pt")
        files = set(glob.glob(new_pattern)) | set(glob.glob(legacy_pattern))
        # Exclude last.pt and any non-step file
        files = [p for p in files if Path(p).stem not in ("last", "best")]
        checkpoints = sorted(
